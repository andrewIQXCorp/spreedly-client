...
PaymentMethods.storePaymentMethod = function storePaymentMethod(paymentMethod, gateway, cb) {
	// Allow caller to pass in payment method objects
	paymentMethod = paymentMethod.token || paymentMethod;

	return this._doRequest({
		url: 'gateways/' + gateway + '/store.json',
		method: 'post',
		body: util.preparePost('transaction', { paymentMethodToken: paymentMethod })
	})
		.then(util.extractRoot())
		.nodeify(cb);
};
